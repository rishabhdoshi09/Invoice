...
module.exports = {
    validateCreatePaymentObj: (paymentObj) => {
        ...
        const schema = Joi.object().keys({
            ...
            partyType: Joi.string().trim().valid('customer', 'supplier', 'expense', 'employee').required(),
            amount: Joi.number().greater(0).max(MAX_PAYMENT_AMOUNT).required(),
            referenceType: Joi.string().trim().valid('order', 'purchase', 'advance', 'salary').required(),
    ...
    validateListPaymentsObj: (paymentObj) => {
        const schema = Joi.object().keys({
            ...
            partyType: Joi.string().trim().valid('customer', 'supplier', 'expense', 'employee').allow("").optional(),
